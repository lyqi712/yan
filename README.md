...
> 当前版本：4.1.6（GitHub `main`）。源自 WxLens Reader Extension 3.0.0 的改进版，显示名称统一为「眼」。微信查询仍依赖独立安装并初始化的 WxLens；本源码不包含 WxLens 安装器或微信账号数据。
>
> **发布状态**：GitHub `main` 已包含 4.1.6 的源码、安装脚本、测试和文档。源码发行包可在本机运行 `npm run package:product` 生成；`dist/` 产物默认不进 Git，当前 GitHub Release/Tag 仍是 v4.1.3。需要 WxLens 的 Windows 整合包时，必须另行准备并核验用户已获授权的第三方安装器。
...
2026-09-25已完成 4.1.6 本地验收：开发树和运行副本均通过62/62项测试，真实WxLens元数据复核通过；这些结果是本地验证，不替代GitHub Actions或全新电脑的扫码、初始化和多客户端接入验收。
