...
function registerArticles({register,result,failure,request,accountContext,store=createArticleStore()}) {
 ...
 register('search_wechat_articles_batch','高强度多轮发现公众号文章：请求间隔1.2秒、最多12次/45秒；验证码或429/403终止整个批次，返回候选、失败和未完成页。时间只过滤已搜到的索引结果，不保证完整历史。',{account_name:z.string().min(1).max(200).optional(),query:z.string().min(1).max(200).optional(),queries:z.array(z.string().min(1).max(200)).max(5).optional(),max_pages:z.number().int().min(1).max(10).optional(),days:z.never().optional().describe('不支持days，请用Unix秒start_time/end_time过滤索引结果'),start_time:time,end_time:time},safe(async p=>result(await searchArticlesBatch(p))))
