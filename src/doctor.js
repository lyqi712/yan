...
const { discoverYan } = require('./yan-runtime')
...
async function diagnose(options = {}) {
  ...
  try {
    ...
    checks.push({ id: 'service', status: 'pass', title: '眼的本机服务', detail: '本地接口可连接；聊天索引仍以首次查询结果为准' })
  } catch (error) { checks.push({ id: 'service', status: 'fail', title: '眼的本机服务', detail: error.message }) }
  const exe = discoverYan(config)
  checks.push({ id: 'desktop', status: exe ? 'pass' : 'optional', title: '眼的启动入口', detail: exe ? (config.autoStart ? '可在查询时自动后台启动' : '已关闭自动启动，请自行打开眼') : '未发现程序；现有服务可直接使用，首次安装请运行setup.cmd' })
  ...
  return { ok: checks.every(item => item.status !== 'fail'), checks, capabilities, configFile: CONFIG_FILE, nextStep: checks.some(item => item.id === 'service' && item.status === 'fail') ? '首次运行setup.cmd，在眼的界面完成微信登录、密钥获取和索引初始化，再运行doctor。日常查询可自动后台启动。' : '将MCP配置合并到AI客户端，重新加载后直接提问；先调用list_sessions定位目标群。', mcpConfigs: renderMcpConfigs() }
}
